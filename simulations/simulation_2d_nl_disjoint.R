...
ES_M_rock_2x = function(n,p,tau,seed){# method = "ES_rf" or "ES_qrf"
  ...
  #y = a1 + x%*%a0 + (x^2%*%b0 + b1)*(rnorm(n)-V0)
  ...
  param.others = list(ds = ds,delta=delta,
    ...
    Res_grid_K = foreach(m_id=1:length(bin_num), .combine='rbind',.inorder = T,
                       .noexport = c('mRock_KNN_both','mRock_KNN_q0_both'))%dopar% {
  ...
  write.table(res_final,file=paste("/home/shushuz/M_rock/results_2d_nl_gaussian/n",n,"p",p,"tau",tau,"seed",seed,".csv",sep=""),sep=",",
                  row.names=FALSE)
  return(0)
}
